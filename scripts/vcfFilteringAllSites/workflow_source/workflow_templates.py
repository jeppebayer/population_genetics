...
def normalize_vcf_all_sites(vcfFile: str, referenceGenomeFile: str, outputDirectory: str):
	"""
	Template: template_description
...
def snpeff_build_database(referenceGenome: str, gtfAnnotation: str, outputDirectory: str, speciesName: str, snpeffConfig: str = f'{os.path.dirname(os.path.realpath(__file__))}/software/snpeff/snpEff.config'):
	...
	inputs = {'reference': referenceGenome,
		   	  'gtf': gtfAnnotation}
	outputs = {'config': f'{outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}',
			   'cds': f'{outputDirectory}/snpeff/data/{os.path.basename(os.path.splitext(referenceGenome)[0])}/cds.fa',
	...
	[ -e {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)} ] || cp {snpeffConfig} {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}

	if [ ! -z "$(grep --line-regexp "# {speciesName} genome, {os.path.basename(referenceGenome)}" {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)})" ]; then
		echo 1>&2 "'# {speciesName} genome, {os.path.basename(referenceGenome)}' is present in {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}"
		echo 1>&2 "Database already exists. If the database entry is not correct please delete it manually and rerun."
		exit 0
	else
		echo "# {speciesName} genome, {os.path.basename(referenceGenome)}" >> {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}
		echo "{os.path.basename(os.path.splitext(referenceGenome)[0])}.genome : {speciesName}" >> {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}
		echo "{os.path.basename(os.path.splitext(referenceGenome)[0])}.file_location : {referenceGenome}" >> {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}
		echo "{os.path.basename(os.path.splitext(referenceGenome)[0])}.addition_date : $(date +%d'/'%m'/'%Y)" >> {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}
		echo -e "{os.path.basename(os.path.splitext(referenceGenome)[0])}.data_directory : {outputDirectory}/snpeff/data/{os.path.basename(os.path.splitext(referenceGenome)[0])}\\n" >> {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)}
	...
	snpEff build \\
		-gtf22 \\
		-config {outputDirectory}/snpeff/{os.path.basename(snpeffConfig)} \\
...
def snpeff_annotation(vcfFile: str, snpeffPredictorFile: str, vcfOutputDirectory: str, snpeffOutputDirectory: str, snpeffConfigFile: str = f'{os.path.dirname(os.path.realpath(__file__))}/software/snpeff/snpEff.config'):
	...
	[ -d {vcfOutputDirectory} ] || mkdir -p {vcfOutputDirectory}
